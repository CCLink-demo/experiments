...
class Token(object):
    ...
    def getFromLinedCode_n(self, code):
        tokenizedCode = []
        rel = []
        for line in code:
            tmp = []
            ...
            for i, w in enumerate(words):
                if w[0] in ['ID', 'ERROR', 'Method']:
                    splittedList = wordninja.split(w[1].lower())
                    tmp += splittedList
                    tmpRel += [i] * len(splittedList)
                else:
                    tmp.append(w[1].lower())
                    tmpRel.append(i)
    ...
    def splitLinesWithFixedLength(self, codes):
        linedCodes = []
        for code in codes:
            code = code[0].strip().split(' ')
            tmp = [code[i: min(i + 6, len(code))] for i in range(0, len(code), 6)]
            tmp = [' '.join(i) for i in tmp]
            linedCodes.append(tmp)
        return linedCodes
